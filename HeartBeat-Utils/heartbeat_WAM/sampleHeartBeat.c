...
void PoC_heartbeat(int nodes_number);
...
void PoC_heartbeat(int nodes_number) {
    ...
    char *data = NULL, prefix_str_index[20]="read_index_", prefix_str_pubK[20]="pub_key_";
    ...
    for(i = 0; i<nodes_number; i++){
        ...
        hex_2_bin(cJSON_GetObjectItemCaseSensitive(json, prefix_str_index)->valuestring, INDEX_HEX_SIZE, read_response_indexes[i].index, INDEX_SIZE);
        hex_2_bin(cJSON_GetObjectItemCaseSensitive(json, prefix_str_pubK)->valuestring, (ED_PUBLIC_KEY_BYTES * 2) + 1, read_response_indexes[i].keys.pub, ED_PUBLIC_KEY_BYTES);
    }   
